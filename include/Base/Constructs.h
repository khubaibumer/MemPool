...
using PoolVec_t = std::vector<PoolNode_t>;
...
typedef struct ObjectPool {
    ...
    ObjectPool(size_t volume, size_t size) {
        totalCount_ = volume;
        size_ = size;
        count_ = 0;
        index_ = 0;
        pool_ = std::make_unique<PoolVec_t>(totalCount_);
        ...
        for ( ; i < totalCount_ ; i++) {
            PoolNode_t node;
            node.inUse_ = false;
            node.data_ = (void *) ((char *) chunkHead_ + (size * i));
            pool_->push_back(node);
        }
        guard_ = ((uint8_t *) chunkHead_ + (size * i));
    }

    ObjectPool() : count_(0), size_(0), index_(0), pool_(nullptr), totalCount_(0), chunkHead_(nullptr),
                   guard_(nullptr) {}
...
struct PointerNode {
    ...
    explicit PointerNode(void *ptr) : ptr_(ptr), _next(ATOMIC_VAR_INIT(nullptr)) {}
